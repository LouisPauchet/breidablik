<template>
  <div v-if="duty">
    <PageHeader title="Edit duty" :back="`/duties/${duty.id}`" />
    <form class="card" @submit.prevent="submit">
      ...
      <fieldset v-if="!originallyOnTeam" class="mode-fieldset">
      ...
      <p v-else class="muted small">
        This duty is attached to a team. To switch it back to manual rotation, delete and
        recreate it instead — you can still move it to a different team below.
      </p>
...
const dutyId = route.params.id as string

await Promise.all([members.ensureLoaded(), teams.fetchTeams(), duties.fetchDuty(dutyId)])

const duty = computed(() => duties.current)

function fromDays(days: number): { value: number; unit: 'days' | 'weeks' } {
  return days % 7 === 0 ? { value: days / 7, unit: 'weeks' } : { value: days, unit: 'days' }
}

const initialTask = fromDays(duty.value?.task_interval_days ?? 7)
const initialRotation = fromDays(duty.value?.rotation_interval_days ?? 14)

const title = ref(duty.value?.title ?? '')
const description = ref(duty.value?.description ?? '')
const taskValue = ref(initialTask.value)
const taskUnit = ref<'days' | 'weeks'>(initialTask.unit)
const rotationValue = ref(initialRotation.value)
const rotationUnit = ref<'days' | 'weeks'>(initialRotation.unit)
const selectedIds = ref<string[]>(
  duty.value ? [...duty.value.assignees].sort((a, b) => a.order_index - b.order_index).map((a) => a.user_id) : []
)
const loading = ref(false)
const error = ref('')

const originallyOnTeam = !!duty.value?.team_id
const mode = ref<'manual' | 'team'>(originallyOnTeam ? 'team' : 'manual')
const teamId = ref(duty.value?.team_id ?? teams.teams[0]?.id ?? '')
...
async function submit() {
  ...
  try {
    ...
    await router.push(`/duties/${dutyId}`)
  } catch {
