<template>
  <div>
    ...
    <form class="card" @submit.prevent="submit">
      <label>
        Name
        <input v-model="name" required placeholder="e.g. Cleaning Team" />
      </label>
      <label>
        Description
      ...
      <label>
        Start date (pick the first "change day", e.g. a Monday)
        <input v-model="startDate" type="date" required />
      </label>

      <fieldset>
        <legend>How often does responsibility change?</legend>
      ...
      <fieldset>
        <legend>Who's on the team? (tap to add, in order)</legend>
...
async function submit() {
  ...
  try {
    const team = await teams.createTeam({
      ...
      member_user_ids: selectedIds.value,
    })
    await router.push(`/duties/teams/${team.id}`)
  } catch {
    error.value = 'Could not create the team. Check the fields and try again.'
  } finally {
    loading.value = false
  }
}
